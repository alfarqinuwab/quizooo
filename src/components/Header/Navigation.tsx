    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 bg-primary-purple border-b-3 border-accent-orange py-0 px-0 w-full transition-all duration-300 ${isScrolled ? 'shadow-lg' : ''}`}>
      <div className="w-full flex items-center justify-between h-16 max-w-full">
        ...
        <div className="flex-1 flex justify-center items-center h-full space-x-8 space-x-reverse">
          ...
          <Link to="/family-challenge" className="text-white font-bold text-lg hover:text-accent-orange transition-colors">نبذة عنا</Link>
          <Link 
            to="/" 
            className="font-bold text-primary-purple text-base px-4 py-1.5 rounded-[2rem] border-3 border-accent-orange bg-accent-orange hover:bg-accent-orange/90 transition-colors"
        ...
        {/* Login (left) */}
        <div className="flex items-center h-full px-6">
          <Link to="/login" className="flex items-center bg-primary-purple text-white rounded-full px-5 py-2 font-bold text-lg hover:bg-primary-purple/80 transition-colors ml-2">
